...
#Identifying signals and daily returns.
buy_signals = {}
sell_signals = {}
for ticker in tickers:
    print("Calculating daily returns and storing signals for", ticker)
    tickers_ret[ticker] = []
    tickers_signal[ticker] = ""
    buy_signals[ticker] = []
    sell_signals[ticker] = []
    ...
    for i in range(len(df)):
        if tickers_signal[ticker] == "":
            ...
            if df["bar_num"].iloc[i] >= 2 and df["obv_slope"].iloc[i] > 30:
                tickers_signal[ticker] = "Buy"
                buy_signals[ticker].append((df.index[i], df["Close"].iloc[i]))

            elif df["bar_num"].iloc[i] <= -2 and df["obv_slope"].iloc[i] < -30:
    ...
    # Assign return column to DataFrame
    df["ret"] = tickers_ret[ticker]
...
#Calculating individual stock KPI's
cagr = {}
sharpe_ratio = {}
max_drawdown = {}
for ticker in tickers:
